...
#[derive(Debug, Clone)]
pub struct WorldView<'a, S: Simulation> {
    population: Box<HashMap<usize, &'a Agent<S::Data>>>,
    data: S::View
}

impl<'a, S: Simulation> WorldView<'a, S> {
    pub fn new(population: Box<HashMap<usize, &'a Agent<S::Data>>>, data: S::View) -> Self {
        Self { population, data }
    }
    pub fn population(&self) -> &HashMap<usize, &'a Agent<S::Data>> {
        &self.population
    }
...
pub trait Simulation: Sized + Send + Sync + Clone {
    ...
    fn worldview<'a>(
        &self,
        _id: usize,
        _population: &'a Self::SimulationPopulation,
        _world: &Self::World,
